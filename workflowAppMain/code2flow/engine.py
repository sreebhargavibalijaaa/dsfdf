...
def write_file(outfile, nodes, edges, groups, hide_legend=False,
    ...
    if not no_grouping:
        for group in groups:
            if len(groups) == 1 and group.group_type == GROUP_TYPE.FILE:
                for subgroup in group.subgroups:
                    content += subgroup.to_dot()
            else:
...
def detect_code_language(code_str):
    """
    Simple heuristic to detect script language from code snippet.
    """
    code_str = code_str.strip()
    if code_str.startswith("<?php"):
        return 'php'
    if any(sig in code_str for sig in ('def ', 'import ', 'class ', '__name__', 'print(')):
        return 'py'
    if any(sig in code_str for sig in ('function ', 'const ', 'let ', 'console.log', 'var ')):
        return 'js'
    return 'py'
...
def get_sources_and_language(raw_source_paths=None, language=None, code=None):
    """
    Given a script file, list of script files, or raw code string, return script files to parse.
    Designed for individual scripts and code snippets rather than full repository trees.

    :param list[str]|str|None raw_source_paths: script file path(s) or raw code string
    :param str|None language: Input language
    :param str|None code: Direct source code string
    :rtype: (list[str], str, list[str])
    """
    temp_files = []
    ...
    if code is not None:
        ...
        tf = tempfile.NamedTemporaryFile(mode='w', suffix=f'.{language}', prefix='script_', delete=False, encoding='utf-8')
        tf.write(code)
        tf.close()
        temp_files.append(tf.name)
        return [tf.name], language, temp_files

    if raw_source_paths is None:
        raise AssertionError("Either 'raw_source_paths' or 'code' must be provided.")

    if isinstance(raw_source_paths, str):
        # Check if raw_source_paths is actually raw code (contains newlines or is not a file on disk)
        if '\n' in raw_source_paths or not os.path.exists(raw_source_paths):
            if any(kw in raw_source_paths for kw in ('def ', 'function ', 'class ', 'import ', 'var ', 'const ', 'let ', '<?php')):
                if not language:
                    language = detect_code_language(raw_source_paths)
                tf = tempfile.NamedTemporaryFile(mode='w', suffix=f'.{language}', prefix='script_', delete=False, encoding='utf-8')
                tf.write(raw_source_paths)
                tf.close()
                temp_files.append(tf.name)
                return [tf.name], language, temp_files
    ...
    for source in sorted(raw_source_paths):
        ...
        if os.path.isdir(source):
            for f in sorted(os.listdir(source)):
                full_p = os.path.join(source, f)
                if os.path.isfile(full_p):
                    ext = f.rsplit('.', 1)[-1].lower() if '.' in f else ''
                    if ext in LANGUAGES:
                        individual_files.append((full_p, False))
            continue
        if '\n' in source or any(kw in source for kw in ('def ', 'function ', 'class ')):
            lang = language or detect_code_language(source)
            tf = tempfile.NamedTemporaryFile(mode='w', suffix=f'.{lang}', prefix='script_', delete=False, encoding='utf-8')
            tf.write(source)
            tf.close()
            temp_files.append(tf.name)
            individual_files.append((tf.name, True))

    if not individual_files:
        raise AssertionError("No script files or code found from %r" % raw_source_paths)
    logging.info("Found %d script file(s).", len(individual_files))

    if not language:
        language = determine_language(individual_files)

    sources = set()
    for source, explicitly_added in individual_files:
        if explicitly_added or source.endswith('.' + language):
            sources.add(source)
        else:
            logging.info("Skipping %r which is not a %s file.", source, language)

    if not sources:
        raise AssertionError(f"Could not find any script files given {raw_source_paths} and language {language}.")

    sources = sorted(list(sources))
    logging.info("Processing %d script file(s)." % (len(sources)))
    for source in sources:
        logging.info("  " + source)

    return sources, language, temp_files
...
def make_file_group(tree, filename, extension):
    ...
    base_name = os.path.split(filename)[-1].rsplit('.' + extension, 1)[0]
    token = 'script' if base_name.startswith('script_') or base_name in ('<script>', 'script') else base_name
    line_number = 0
    display_name = 'Script'
...
def code2flow(raw_source_paths=None, output_file=None, code=None, language=None,
              hide_legend=True, exclude_namespaces=None, exclude_functions=None,
    ...
    """
    Top-level function. Generate a flowchart diagram based on script code or file.
    Works directly with code snippets or script files.

    :param list[str]|str|None raw_source_paths: script file or list of script files, or raw code string
    :param str|file|None output_file: path to the output file (e.g. .png, .svg, .gv, .json). If None, returns DOT string.
    :param str|None code: direct source code string
    :param str|None language: input language extension ('py', 'js', 'rb', 'php')
    ...
    sources, language, temp_files = get_sources_and_language(
        raw_source_paths=raw_source_paths, language=language, code=code
    )
    ...
    try:
        ...
        if output_file is None:
            buf = io.StringIO()
            write_file(buf, nodes=all_nodes, edges=edges,
                       groups=file_groups, hide_legend=hide_legend,
                       no_grouping=no_grouping, as_json=False)
...
def flow_from_code(code, language=None, output_file=None, hide_legend=True, **kwargs):
    """
    Generate flowchart directly from code string.
    Returns DOT string if output_file is None, otherwise writes to output_file.
    """
    return code2flow(code=code, output_file=output_file, language=language,
...
def main(sys_argv=None):
    ...
    parser.add_argument(
        ...
        help='source script file(s).')
    parser.add_argument(
        '--code', '-c', default=None,
        help='raw script code string to parse directly.')
    parser.add_argument(
    ...
    code2flow(
        raw_source_paths=args.sources if args.sources else None,
